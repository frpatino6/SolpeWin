...
export class HomeService {
    ...
    private serverUrl = "http://190.24.154.2:446/solpe/GetLiberaSolpes/";
    private serverDatabaseUrl = "https://190.24.154.2:446/solpe/";
    ...
    getOrders(user: String): Observable<Orders[]> {
        ...
        return this.http.get<Orders[]>(this.serverUrl + user, {
            headers: headers
        });
    }
    updateOrdersSolpe(number: string, pos: number): Observable<Orders> {
        ...
        return this.http.post<Orders>(
            this.serverDatabaseUrl + "UpdateSolpeState/" + number + "/" + pos,
            { headers: headers }
        );
    }
    updatePedidoState(number: string): Observable<Orders> {
        let headers = this.createRequestHeader();
        return this.http.post<Orders>(
            this.serverDatabaseUrl + "UpdatePedidoState/" + number,
            { headers: headers }
        );
    }
